...
func (mssql *SQLServer) GetUserById(ctx context.Context, id string) (user *models.User, err error) {
	// preparing statement
...
// GetInfoUserByIdAndUserId get multiple rows from db in info_Users by its id, user_id
func (mssql *SQLServer) GetInfoUserByIdAndUserId(ctx context.Context, id string, user_id string) (infoUser *models.InfoUser, err error) {
	// preparing statement
	...
	defer CloseStatement(stmt)
	// Query for a value based on a single row.
	if err := stmt.QueryRow(id, user_id).
		Scan(&infoUser.Id,
			&infoUser.Phone,
			&infoUser.Country,
			&infoUser.CodCountry,
...
// GetQueryValueByUrlId get valueParams by its urlId
func (mssql *SQLServer) GetQueryValueByUrlId(ctx context.Context, url_id string) (queryValues []*models.QueryValue, err error) {
	// preparing statement
	query := `SELECT id, key_param, url_id
	FROM query_keys
	WHERE url_id = @p1
